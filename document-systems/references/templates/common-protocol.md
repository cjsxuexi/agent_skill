<!--
Template for a `protocol` common document (public protocol).
...
Placeholders:
  ...
  <SCOPE_BODY>  one sentence: which subsystems / repos speak this protocol, at which level
                (engine generates from the op's source list + level)
  <TYPE_BODY>   the protocol definition + producers / consumers (from body_file)
...
`## 2. 协议定义与生产/消费方`, and `## 待确认 / 疑问`. Prose inside each section is free.
-->
---
common_type: protocol
level: <LEVEL>
owns: <OWNS>
---
